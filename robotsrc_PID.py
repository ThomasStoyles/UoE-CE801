...
def PID(): 
    ...
    while True:
        actual_distance = min(regions['front2'], regions['front1'], regions['fright']) # Sensor distance 
        disered_distance = 0.5 # disered distance 
        ep = disered_distance - actual_distance # disered_distance - actual_distance
        all_errors.append(ep)# adding errors to list 
        ed = (all_errors[-2] - all_errors[-1]) # last error - previous error
        ei = sum(all_errors) + ep # sum of errors + ep 
